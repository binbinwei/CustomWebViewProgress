...
/*********************************************************
 ...
 自定义WebView(实现加载进度显示、加载错误处理、加载超时处理)
 *********************************************************/
public class LoadWebView extends WebView {
    ...
    /** 刷新背景颜色*/
    private static final int ERRO_BACKGROUND = 0xFFFFFFFF;
    /** 进度条默认的高度*/
    private static final int DEF_BAR_HEIGHT = 2;// dp
    private static final String DEF_ERROR_TEXT = "刷新\n\n请检查你的网络或点击刷新";
    /** 错误显示文字颜色*/
    private static final int DEF_ERROR_TEXT_COLOR = 0xFF808080;// 红色
    /** 错误显示文字字体大小*/
    private static final int DEF_ERROR_TEXT_SIZE = 10;// sp
    ...
    private void obtainStyledAttributes(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (TextUtils.isEmpty(mErrorText)) mErrorText = DEF_ERROR_TEXT;
    ...
    private void addErrorTextView() {
        ...
        mTextDes = new TextView(mContext);
        ...
        mTvError.setBackgroundColor(ERRO_BACKGROUND);
        ...
        mTextDes.setVisibility(INVISIBLE);
        mTextDes.setText(mErrorText);
        mTextDes.setTextSize(mErrorTextSize);
        mTextDes.setTextColor(mErrorTextColor);
...
//    }
    ...
    private void initProgressBar() {
        ...
        mProgressBar.setLayoutParams(new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.MATCH_PARENT, mBarHeight));
    ...
    private void hideErrorTextView() {
        if (mTvError != null && mTvError.getVisibility() == VISIBLE&&flag)
            mTvError.setVisibility(INVISIBLE);
    }
